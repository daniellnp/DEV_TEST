                            ...
                            <div class="ml-12">
                                <div class="mt-2 text-gray-600 dark:text-gray-400 text-sm">
                                    @if(Request::is('*/edit'))
                                        ...
                                        @csrf
                                        <input class="hidden" style='border:1px solid black;' name='id' size='' maxlength='' value="{{$edit->id}}">
                                        CEP: <input class="input" style='border:1px solid black;' name='cep' value="{{$edit->cep}}" type="text" v-model="cep" @keyup="searchCep()" size='' maxlength='' ><br>
                                        Estado: <select id="estado" name="estado" class='input'style='border:1px solid black;' >
                                                <option value="" >Selecione um Estado</option>
                                                
                                                <option value="AC" {{($edit->estado=='AC')?'selected':'';}}>ACRE</OPTION>
                                                <option value="AL" {{($edit->estado=='AL')?'selected':'';}}>ALAGOAS</OPTION>
                                                <option value="AP" {{($edit->estado=='AP')?'selected':'';}}>AMAPÁ</OPTION>
                                                <option value="AM" {{($edit->estado=='AM')?'selected':'';}}>AMAZONAS</OPTION>
                                                <option value="BA" {{($edit->estado=='BA')?'selected':'';}}>BAHIA</OPTION>
                                                <option value="CE" {{($edit->estado=='CE')?'selected':'';}}>CEARÁ</OPTION>
                                                <option value="DF" {{($edit->estado=='DF')?'selected':'';}}>DISTRITO FEDERAL</OPTION>
                                                <option value="ES" {{($edit->estado=='ES')?'selected':'';}}>ESPÍRITO SANTO</OPTION>
                                                <option value="GO" {{($edit->estado=='GO')?'selected':'';}}>GOIÁS</OPTION>
                                                <option value="MA" {{($edit->estado=='MA')?'selected':'';}}>MARANHÃO</OPTION>
                                                <option value="MT" {{($edit->estado=='MT')?'selected':'';}}>MATO GROSSO</OPTION>
                                                <option value="MS" {{($edit->estado=='MS')?'selected':'';}}>MATO GROSSO DO SUL</OPTION>
                                                <option value="MG" {{($edit->estado=='MG')?'selected':'';}}>MINAS GERAIS</OPTION>
                                                <option value="PA" {{($edit->estado=='PA')?'selected':'';}}>PARÁ</OPTION>
                                                <option value="PB" {{($edit->estado=='PB')?'selected':'';}}>PARAÍBA</OPTION>
                                                <option value="PR" {{($edit->estado=='PR')?'selected':'';}}>PARANÁ</OPTION>
                                                <option value="PE" {{($edit->estado=='PE')?'selected':'';}}>PERNAMBUCO</OPTION>
                                                <option value="PI" {{($edit->estado=='PI')?'selected':'';}}>PIAUÍ</OPTION>
                                                <option value="RJ" {{($edit->estado=='RJ')?'selected':'';}}>RIO DE JANEIRO</OPTION>
                                                <option value="RN" {{($edit->estado=='RN')?'selected':'';}}>RIO GRANDE DO NORTE</OPTION>
                                                <option value="RS" {{($edit->estado=='RS')?'selected':'';}}>RIO GRANDE DO SUL</OPTION>
                                                <option value="RO" {{($edit->estado=='RO')?'selected':'';}}>RONDÔNIA</OPTION>
                                                <option value="RR" {{($edit->estado=='RR')?'selected':'';}}>RORAIMA</OPTION>
                                                <option value="SC" {{($edit->estado=='SC')?'selected':'';}}>SANTA CATARINA</OPTION>
                                                <option value="SP" {{($edit->estado=='SP')?'selected':'';}}>SÃO PAULO</OPTION>
                                                <option value="SE" {{($edit->estado=='SE')?'selected':'';}}>SERGIPE</OPTION>
                                                <option value="TO" {{($edit->estado=='TO')?'selected':'';}}>TOCANTINS</OPTION>
                                                <option value="EX" {{($edit->estado=='EX')?'selected':'';}}>ESTRANGEIRO</OPTION>
                                            </select>
                                            <br/> 

                                        Cidade: <input class='input' style='border:1px solid black;' name='cidade' size='' maxlength='' value="{{$edit->cidade}}"><br>
                                        Bairro: <input class='input' style='border:1px solid black;' name='bairro' size='' maxlength='' value="{{$edit->bairro}}"><br>
                                        Endereço: <input class='input' style='border:1px solid black;' name='endereco' size='' maxlength='' value="{{$edit->endereco}}"><br>
                                        Numero: <input class='input' style='border:1px solid black;' name='numero' size='' maxlength='' value="{{$edit->numero}}"><br>
                                        Complemento: <input class='input' style='border:1px solid black;' name='complemento' size='' maxlength='' value="{{$edit->complemento}}"><br>
                                        Contato: <input class='input' style='border:1px solid black;' name='name' size='' maxlength='' value="{{$edit->name}}"><br>
                                        E-mail: <input class='input' style='border:1px solid black;' name='email' size='' maxlength='' value="{{$edit->email}}"><br>
                                        Telefone: <input class='input' style='border:1px solid black;' name='telefone' size='' maxlength='' value="{{$edit->telefone}}"><br><p>
                                        ...
                                        <a  style='background:gray; color:white; box-shadow: 1px 1px 2px #333333; height:45px; border:1px solid black;' href="{{url('/cad/')}}">Cancelar</a>
                                    ...
                                    @if(!empty($contatos))
                                        ...
                                        @foreach ($contatos as $con)
                                         <tbody >
                                            ...
                                            <td scope="col" >{{$con->telefone}}</td>
                                            <td scope="col" >{{$con->cep}}</td>
                                            <td scope="col" >{{$con->estado}}</td>
                                            <td scope="col" >{{$con->cidade}}</td>
                                            <td scope="col" >{{$con->bairro}}</td>
                                            <td scope="col" >{{$con->endereco}}</td>
                                            <td scope="col" >{{$con->numero}}</td>
                                            <td scope="col" >{{$con->complemento}}</td>
                                            <th scope="col"><a style='background:gray; color:white; box-shadow: 1px 1px 2px #333333; height:25px; border:1px solid black;'  href="{{url($con->id.'/edit/')}}">Editar</a></th>
                                            <th scope="col">
                                               

                                                <form name='form' method=post action="{{url($con->id.'/delete/')}}">
                                                @csrf
                                                @method('delete')
